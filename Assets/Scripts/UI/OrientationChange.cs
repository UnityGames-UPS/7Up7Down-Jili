...
public class OrientationChange : MonoBehaviour
{
  ...
  private void ApplyMatch(int width, int height)
  {
    ...
    if (UIWrapper != null) rotationTween = UIWrapper.DOLocalRotateQuaternion(targetRotation, transitionDuration).SetEase(Ease.OutCubic);
    ...
    if (CanvasScaler != null) matchTween = DOTween.To(() => CanvasScaler.matchWidthOrHeight, x => CanvasScaler.matchWidthOrHeight = x, targetMatch, transitionDuration).SetEase(Ease.InOutQuad);
  }
